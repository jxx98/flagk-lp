...
function cta_label($id)
{
    $labels = [
        ...
        'header_tel'          => 'ヘッダー：お電話で出店申込',
        'header_line'         => 'ヘッダー：LINE相談',
        'hero_apply'          => 'ヒーロー：Uber Eatsの出店申込',
        'flow_contact'        => '出店申込から店舗登録までの流れ：問い合わせる',
        'stats_tel'           => '実績セクション：出店申込フォームへ',
        'final_campaign'      => '最終CTA：キャンペーン確認',
        'final_tel'           => '最終CTA：出店相談予約',
        'final_line'          => '最終CTA：LINE相談',
        'mobile_bar_tel'      => 'モバイル下部バー：出店相談予約',
...
switch ($type) {
    ...
    case 'tel':
        ...
        $reply_to_email = $email !== '' ? $email : '';
        $times     = (isset($_POST['contact_time']) && is_array($_POST['contact_time']))
                     ? array_map('clean', $_POST['contact_time'])
                     : [];

        if ($shop_name === '' || $contact === '' || $tel === '') {
            header('Location: /ubereats-support/tel-reservation/');
            exit;
        }

        $body .= "店舗名・会社名：{$shop_name}\n";
        $body .= "担当者名　　　：{$contact}\n";
        $body .= "電話番号　　　：{$tel}\n";
        $body .= "メール　　　　：" . ($email !== '' ? $email : '（未入力）') . "\n";
        $body .= "希望連絡時間帯：" . (!empty($times) ? implode('、', $times) : '（未選択）') . "\n";
        $body .= "質問・備考　　：" . ($message !== '' ? $message : '（なし）') . "\n";
        break;
